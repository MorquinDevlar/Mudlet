 ...
 * for the same reason.
 *
 * Four things keep an ink of their own, and each is left out for a reason
 ...
 *   them.
 * - The two state chips, by object name. Both are drawn in a colour that says
 *   a reading rather than a tone - the compile chip in a state hue walked
 *   against its own fill, the OR/AND chip in the accent while it is the mode in
 *   force - so neither is chrome in the sense this measures.
...
private:
    ...
    // Measured, and pinned under what the walk actually reaches - 30 things on
    // the dark appearance and the same on the light - so that a walk which
    // stopped finding widgets cannot pass as a walk that found them all in the
    // one ink
    static constexpr int scmLeastAudited = 24;
    ...
    // The two chips whose colour says a reading rather than a tone: the compile
    // state, drawn in a state hue walked against its own fill, and the OR/AND
    // chip, which takes the accent while it is the mode in force. Named here
    // because both are drawn as a widget with words inside it, and the words
    // are what would otherwise be measured.
    static bool aStateChip(const QWidget* pWidget)
    {
        for (const QWidget* pAt = pWidget; pAt; pAt = pAt->parentWidget()) {
            if (pAt->objectName() == qsl("editorCompileChip") || pAt->objectName() == qsl("editorModeChip")) {
                return true;
            }
        }
        return false;
    ...
    int walkTheChrome(const QString& appearance, const QColor& chrome, QStringList& misses) const
    {
        ...
        for (const QWidget* pWidget : mpEditor->findChildren<QWidget*>()) {
            ...
            if (insideAField(pWidget) || aStateChip(pWidget)) {
                continue;
            }
...
private slots:
    ...
    // One case for both appearances and one list at the end: a run that stopped
    // at the first word in the wrong tone would take as many runs to clear as
    // there are of them
    void test_everyWordOutsideAFieldIsTheOneQuietTone()
    {
        ...
        for (const auto& appearance : QList<QPair<QString, enums::Appearance>>{{qsl("dark"), enums::Appearance::dark}, {qsl("light"), enums::Appearance::light}}) {
            ...
            const int audited = walkTheChrome(appearance.first, tokens.mutedText, misses);
